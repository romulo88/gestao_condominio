...
- **Notificações por e-mail**: resolvido (v171) - aprovação, reprovação, finalização no Kanban, todas por e-mail (SMTP configurado via `EmailService`/`EmailTemplates`).
- **Fluxo de troca de senha por e-mail de verdade**: resolvido (v172) - "Esqueci minha senha" (`POST /api/auth/esqueci-senha`) agora gera um código numérico aleatório, grava como a senha da pessoa e manda por e-mail; o passo 2 (`/trocar-senha`) usa esse código como "senha atual". Mais forte que a versão anterior (que confirmava só CPF+e-mail e deixava trocar usando a senha padrão, conhecida por qualquer um) - ainda não é o ideal (um token de reset separado, sem tocar na senha real até a troca de fato, evitaria o efeito colateral de "alguém mais souber CPF+e-mail consegue trocar a senha da pessoa sem ela pedir" - ver `AuthService.esqueciSenha`). `/api/auth/definir-senha` (primeiro acesso de pessoa nova) continua com a mesma limitação de antes - fora do escopo desta rodada.
